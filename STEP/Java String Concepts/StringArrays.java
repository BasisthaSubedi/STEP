...
public class StringArrays {
    public static String findLongestName(String[] names) {
        ...
        for (String name : names) if (name.length() > longest.length()) longest = name;
        return longest;
    }
    public static int countNamesStartingWith(String[] names, char letter) {
        int count = 0;
        for (String name : names) if (Character.toLowerCase(name.charAt(0)) == Character.toLowerCase(letter)) count++;
        return count;
    }
